...
def test_calculate_item_total_price_converts_discount(expected_discount: float = 0.2) -> tuple[bool, str]:
    ...
    total_price = item.calculate_item_total_price()
    actual_discount = 1.0-total_price/(item.unit_price*item.quantity)
    if actual_discount == item.discount_percent:
        fail_case = True
    else:
        fail_case = False
...
def test_calculate_item_total_price_correct() -> tuple[bool, str]:
    ...
    # - Run the test before implementing the calculate_item_total_price method and check that it fails
    # - TIP: Use the round function to round the result to 2 decimal places
    item = Item("Item 1", 1, 1.0, 0.2)  # Replace None with the correct code
    expected_item_price = item.quantity * item.unit_price * (1 - item.discount_percent)# Replace None with the correct code
    actual_item_price = item.calculate_item_total_price() # Replace None with the correct code
    if expected_item_price == actual_item_price:
        fail_case = False
    else:
        fail_case = True
